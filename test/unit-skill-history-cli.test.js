...
function createAllDeleteFixture() {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "codex-history-skill-delete-all-"));
  const sessionsDir = path.join(tempRoot, "sessions", "2026", "02", "09");
  const archivedDir = path.join(tempRoot, "archived_sessions");
  ensureDir(sessionsDir);
  ensureDir(archivedDir);

  const activeSessionId = "sess-delete-all-active";
  const archivedSessionId = "sess-delete-all-archived";
  const duplicateSessionId = "sess-delete-all-duplicate";
  const activeFile = path.join(sessionsDir, "active.jsonl");
  const archivedFile = path.join(
    archivedDir,
    "rollout-2026-02-09T00-00-00-sess-delete-all-archived.jsonl"
  );
  const duplicateActiveFile = path.join(sessionsDir, "duplicate-active.jsonl");
  const duplicateArchivedFile = path.join(
    archivedDir,
    "rollout-2026-02-09T00-00-00-sess-delete-all-duplicate.jsonl"
  );
  const invalidFile = path.join(sessionsDir, "invalid-without-meta.jsonl");

  const writeSession = (filePath, sessionId, title) => {
    fs.writeFileSync(
      filePath,
      [
        JSON.stringify({
  ...
  writeSession(activeFile, activeSessionId, "active delete all");
  writeSession(archivedFile, archivedSessionId, "archived delete all");
  writeSession(duplicateActiveFile, duplicateSessionId, "duplicate active delete all");
  writeSession(duplicateArchivedFile, duplicateSessionId, "duplicate archived delete all");
  fs.writeFileSync(invalidFile, "not json\n", "utf8");

  fs.writeFileSync(
    path.join(tempRoot, "history.jsonl"),
    [
      JSON.stringify({ session_id: activeSessionId, ts: 1707436800, text: "active title" }),
      JSON.stringify({ session_id: archivedSessionId, ts: 1707436801, text: "archived title" }),
      JSON.stringify({ session_id: duplicateSessionId, ts: 1707436802, text: "duplicate title" }),
      JSON.stringify({ session_id: "history-only", ts: 1707436803, text: "history only title" }),
    ].join("\n") + "\n",
    "utf8"
  );
...
test("parseListArgs 支持 detail 输出格式", () => {
  const parsed = cli.parseListArgs(["--format", "detail"]);
  assert.equal(parsed.format, "detail");
});
...
test("parseListArgs 非法 format 抛参数错误", () => {
  assert.throws(
    () => cli.parseListArgs(["--format", "bad"]),
    (error) => error.exitCode === cli.EXIT_CODES.PARAM_ERROR
  );
});
...
test("parseDeleteArgs 支持 all 删除", () => {
  const parsed = cli.parseDeleteArgs(["--all", "--force"]);
  assert.equal(parsed.all, true);
  assert.equal(parsed.force, true);
  assert.deepEqual(parsed.sessionIds, []);
});

test("parseDeleteArgs 禁止 all 与 session-id 混用", () => {
  assert.throws(
    () => cli.parseDeleteArgs(["--all", "--session-id", "a"]),
    (error) => error.exitCode === cli.EXIT_CODES.PARAM_ERROR
  );
});
...
test("runCli list 人类可读输出默认为表格并包含标题字段", () => {
  const fixture = createCodexFixture();
  const originalCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = fixture.tempRoot;
  ...
  try {
    withCapturedStdStreams(({ logs, errors }) => {
      ...
      assert.ok(text.includes("| 来源 | 时间 | 标题 | sessionId | 文件 |"));
      assert.ok(text.includes("历史标题"));
      assert.ok(text.includes(fixture.sessionId));
      assert.ok(text.includes(fixture.sessionFile));
    });
  } finally {
    if (originalCodexHome === undefined) {
      delete process.env.CODEX_HOME;
...
test("runCli list --format detail 保留详细输出", () => {
  const fixture = createCodexFixture();
  const originalCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = fixture.tempRoot;
  ...
  try {
    withCapturedStdStreams(({ logs, errors }) => {
      const code = cli.runCli(["list", "--format", "detail", "--limit", "5"]);
      assert.equal(code, cli.EXIT_CODES.OK);
      assert.equal(errors.length, 0);
...
test("runCli delete --all --json 删除全部会话文件并清空 history.jsonl", () => {
  const fixture = createAllDeleteFixture();
  const originalCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = fixture.tempRoot;

  try {
    withCapturedStdStreams(({ logs, errors }) => {
      const code = cli.runCli(["delete", "--all", "--force", "--json"]);
      assert.equal(code, cli.EXIT_CODES.OK);
      assert.equal(errors.length, 0);

      const payload = JSON.parse(logs.join("\n"));
      assert.equal(payload.all, true);
      assert.equal(payload.sessionCount, 4);
      assert.equal(payload.sessionFileCount, 5);
      assert.equal(payload.deletedFiles, 5);
      assert.equal(payload.failedFileCount, 0);
      assert.equal(payload.removedHistoryLines, 4);
      assert.deepEqual(payload.sessionIds.sort(), [
        "invalid-without-meta",
        "sess-delete-all-active",
        "sess-delete-all-archived",
        "sess-delete-all-duplicate",
      ]);

      assert.ok(!fs.existsSync(fixture.activeFile));
      assert.ok(!fs.existsSync(fixture.archivedFile));
      assert.ok(!fs.existsSync(fixture.duplicateActiveFile));
      assert.ok(!fs.existsSync(fixture.duplicateArchivedFile));
      assert.ok(!fs.existsSync(fixture.invalidFile));
      assert.equal(fs.readFileSync(path.join(fixture.tempRoot, "history.jsonl"), "utf8"), "");
    });
  } finally {
    if (originalCodexHome === undefined) {
      delete process.env.CODEX_HOME;
...
test("runCli delete --all 未传 --force 返回参数错误", () => {
  const fixture = createAllDeleteFixture();
  const originalCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = fixture.tempRoot;
  ...
  try {
    withCapturedStdStreams(({ errors }) => {
      const code = cli.runCli(["delete", "--all"]);
      assert.equal(code, cli.EXIT_CODES.PARAM_ERROR);
      assert.equal(errors.length, 1);
