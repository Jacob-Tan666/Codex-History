...
function buildFallbackFileEntry(filePath, source) {
  let timestamp = "";
  try {
...
function loadAllSessionFileEntries(codexHome) {
  const { map } = readHistoryMap(codexHome);
  const { sessions, archived } = listSessionFiles(codexHome);
  const all = [];
  ...
  const addEntries = (filePaths, source) => {
    for (const filePath of filePaths) {
      const content = readFileIfExists(filePath);
      if (!content) {
        all.push(buildFallbackFileEntry(filePath, source));
        continue;
      }
      const items = parseJsonLines(content);
      const entry = buildSessionEntry({
...
function deleteSessions({ sessionIds, sessionFiles, codexHome, clearHistory = false }) {
  ...
  if (fs.existsSync(historyPath) && clearHistory) {
    const content = readFileIfExists(historyPath);
    const lines = content.split(/\r?\n/).filter((line) => line.trim());
    removedHistoryLines = lines.length;
    fs.writeFileSync(historyPath, "", "utf8");
  } else if (fs.existsSync(historyPath) && ids.size > 0) {
    const content = readFileIfExists(historyPath);
    const lines = content.split(/\r?\n/).filter((line) => line.trim());
    const kept = [];
