...
function formatListHuman(payload) {
  ...
  if (payload.format !== "detail") {
    lines.push("| 来源 | 时间 | 标题 | sessionId | 文件 |");
    lines.push("| --- | --- | --- | --- | --- |");
    for (const item of items) {
      ...
      lines.push(
        `| ${formatTableCell(sourceText)} | ${formatTableCell(timeText)} | ${formatTableCell(
          titleText
        )} | ${formatTableCell(item.sessionId)} | ${formatTableCell(item.filePath)} |`
      );
    }
    return lines.join("\n");
  }
