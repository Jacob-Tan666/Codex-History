...
### Added
- 新增 `ui` 命令，启动本地 Web 可视化控制台。
- 可视化界面支持搜索、来源筛选、会话预览、Markdown 导出、归档、恢复和删除操作。

### Changed
- 打包脚本会把 `public/` 前端资源与 `ui-server.js` 一并放入 Skill 压缩包。
- README 与 `SKILL.md` 同步补充本地可视化界面说明与白名单命令。
